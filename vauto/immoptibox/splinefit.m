...
% BC  :  If present and nonempty, it must either be a 
%        scalar:  Periodic spline if BC=1, otherwise a natural spline,
...
%                     7 :  When given, BC should be either a 
...
% Version 04.05.05.  hbn(a)imm.dtu.dk
...
if  nargin > 2 & ~isempty(BC)
  sbc = size(BC);
  if  all(sbc == 1)            % scalar input
    if  BC == 1                % periodic spline
      period = 1;  ns = n;
    else                       % natural spline
      BC = [0 0 1 0; 0 0 1 0];  sbc = size(BC);
    end
  end
  if  ~period
    if  any(sbc - [2 4]),  S.fail = 7;  return,  end

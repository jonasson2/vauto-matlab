...
%        an empty L is treated as L = I, the unit matrix.
...
%                 Default: standard Huber, ie abs(ri)<=gamma is active.
% init:  Choice of starting point,
%        given x0 if init is a vector,
%        given active set and factorization if init is a struct like 
%            info below.
...
% info :  Struct with information on the performance and computed
%         solution.  Fields
%         info.pf = [F(x)  ||F'(x)||inf  #iterations #factorizations]
%             where x is the computed minimizer.
%             info.pf(1) = -Inf  indicates an unbounded problem.
%         info.S :  Struct with the Huber active set at the solution.  
%             Fields
%             S.s  Huber sign vector,
%             S.A  active set (indices of small residuals),
%             S.N  indices of large residuals,
%             S.L  = [length(S.A), length(S.N)].
%         info.R :  n*n array with Cholesky factor of active set.
%         info.p :  Permutation vector used in the factorization.
% perf :  Array, holding 
%         perf(1,:) = values of  F(x)
%         perf(2,:) = values of  || F'(x) ||inf
%         perf(3,:) = number of active elements.
%
% Method
...
% Version 05.09.30.  hbn(a)imm.dtu.dk
...
if  Trace,  X = X(:,1:k);  perf = perf(:,1:k); 
else,       X = x; end
...
if  nargout > 1  % gamma*gradient
  ...
  thr = 100*eps*(aux(7)*norm(f,inf) + aux(7)*norm(x,inf) + aux(8));

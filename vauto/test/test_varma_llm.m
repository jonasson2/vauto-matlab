...
function test_varma_llm(varargin)
  ...
  fprintf('TESTING VARMA_LLM');
  rand_init(1);
  if full, fprintf(' (COMPREHENSIVE CHECK)'); end
  ...
  fmt = '  Testcase %-17s max diff.=%.1e, max res.diff.=%.1e\n';
  for j = 1:length(cases)
    ...
    elseif  full,      misspat = [0 -1 -2 -3 -4 -5 -6 -7 2 4 10 20 30]; end
    misspat = [0 -1 -2 -3 -4 -5 -6 -7 2 4 10 20 30];
    ...
    for n1 = [0 4 12]
      ...
      for i = misspat
        ...
        ascertain(ok);
        ascertain(almostequal(l, l1));
        if j < 6 % CHECK THAT EMPTY MU RETURNS THE SAME AS ZERO MU:
          zmu = zeros(r,1);
          [l0, ok0] = varma_llm(X1, A, B, Sig, zmu, miss);
          [le, oke] = varma_llm(X1, A, B, Sig, [], miss);
          ascertain(ok0 && oke && almostequal(l0, le));
          [l0, ok0, ld0] = varma_llm(X1, A, B, Sig, zmu, miss);
          [le, oke, lde] = varma_llm(X1, A, B, Sig, [], miss);
          ascertain(ok0 && oke)
          ascertain(length(ld0) == (p+q)*r^2+r*(r+1)/2+r)
          ascertain(length(lde) == (p+q)*r^2+r*(r+1)/2)
          ascertain(almostequal(ld0(1:(p+q)*r^2+r*(r+1)/2), lde));
        end
  ...
  if full, [ltol,rtol,xtol] = deal(1e-14, 2e-13, 1e-13);
  else     [ltol,rtol,xtol] = deal(2e-15, 2e-14, 3e-14); end
  ascertain(max(maxdiff) < ltol);
  ascertain(max(rdiff)   < rtol);
  ascertain(max(mdiff)   < xtol);
  disp('  OK');
end

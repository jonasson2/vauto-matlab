...
%
%   ASSERT_EQUAL(..., MSG) prints MSG in addition to the default message when
%   the assertion fails.
%
%   MAXDIFF = ASSERT_EQUAL(...) returns the maximum absolute/relative difference
%   which was found.

function diff = assert_equal(x, y, tol, extramsg)
  if nargin == 3 && ischar(tol), extramsg = tol; tol = 0;
  elseif nargin == 3; extramsg = '';
  elseif nargin < 3, tol = 0; extramsg = '';
  end
  close = false;
  endl = char(10);
  vname = inputname(1);
  ...
  if ~isempty(extramsg), msg = [endl '(' extramsg ')']; end
  if ~close, assert(false, msg); end
  if nargout > 0, diff = maxdiff; end
end

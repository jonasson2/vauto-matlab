...
%   TEST_VARMA_SIM QUIET prints only the max difference and only for cases
%     1, 3, 5 and 7.
...
function test_varma_sim(varargin)
  ...
  for i=tcase
    ...
    d1 = relabsdiff(CC1, CC);
    d2 = relabsdiff(CC2, CC);
    dmax = max([dmax,d1,d2]);
    if ~quiet
    ...
    d1 = relabsdiff(Ex1, mean(X1,2)); % difference one step ahead
    d2 = relabsdiff(Ex2, mean(X2,2)); % difference two steps ahead
    d3 = relabsdiff(Vx1, cov(X1'));   % x1 covar difference
    if ~quiet
...
function rd = relabsdiff(a, b)
  % max(relative diff, absolute diff) where diff is difference between 
  % vectors a and b
  a = a(:);  b = b(:);
  rd = norm(a - b, inf)/max([1, norm(a,inf), norm(b,inf)]);
end
